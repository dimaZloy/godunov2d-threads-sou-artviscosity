...
function computeInterfaceSlopeFast(i::Int64, k::Int64, testMesh::mesh2d, testFields::fields2d, 
	uLeftp::Array{Float64,1},uUpp::Array{Float64,1},uDownp::Array{Float64,1}, uRightp::Array{Float64,1}, flux::Array{Float64,1})
	...
	if (ek >=1 && ek<=testMesh.nCells)
		...
		if (testMesh.mesh_connectivity[i,2] == 3) ## tri element 
		...
		elseif (testMesh.mesh_connectivity[i,2] == 2) ## quad element 
		...
		## update !!!!
		ComputeUPhysFromBoundariesFast(i,k, ek, uLeftp, nx,ny,uRightp);
	...
	# UpLeft[1]  = uLeftp[1] + 0.5*Minmod_LimiterA( uLeftp[1]-uDownp[1], uRightp[1]-uLeftp[1], ksi);
	# UpLeft[2]  = uLeftp[2] + 0.5*Minmod_LimiterA( uLeftp[2]-uDownp[2], uRightp[2]-uLeftp[2], ksi);
	# UpLeft[3]  = uLeftp[3] + 0.5*Minmod_LimiterA( uLeftp[3]-uDownp[3], uRightp[3]-uLeftp[3], ksi);
	# UpLeft[4]  = uLeftp[4] + 0.5*Minmod_LimiterA( uLeftp[4]-uDownp[4], uRightp[4]-uLeftp[4], ksi);
					
	# UpRight[1] = uRightp[1] - 0.5*Minmod_LimiterA( uRightp[1]-uLeftp[1], uUpp[1]-uRightp[1],  ksi);
	# UpRight[2] = uRightp[2] - 0.5*Minmod_LimiterA( uRightp[2]-uLeftp[2], uUpp[2]-uRightp[2],  ksi);	
	# UpRight[3] = uRightp[3] - 0.5*Minmod_LimiterA( uRightp[3]-uLeftp[3], uUpp[3]-uRightp[3],  ksi);
	# UpRight[4] = uRightp[4] - 0.5*Minmod_LimiterA( uRightp[4]-uLeftp[4], uUpp[4]-uRightp[4],  ksi);
												

	uLeftp[1] +=  0.5*Minmod_LimiterA( uLeftp[1]-uDownp[1], uRightp[1]-uLeftp[1], 1.0e-6 );
	uLeftp[2] +=  0.5*Minmod_LimiterA( uLeftp[2]-uDownp[2], uRightp[2]-uLeftp[2], 1.0e-6 );
	uLeftp[3] +=  0.5*Minmod_LimiterA( uLeftp[3]-uDownp[3], uRightp[3]-uLeftp[3], 1.0e-6 );
	uLeftp[4] +=  0.5*Minmod_LimiterA( uLeftp[4]-uDownp[4], uRightp[4]-uLeftp[4], 1.0e-6 );
					
	uRightp[1] -= 0.5*Minmod_LimiterA( uRightp[1]-uLeftp[1], uUpp[1]-uRightp[1],  1.0e-6 );
	uRightp[2] -= 0.5*Minmod_LimiterA( uRightp[2]-uLeftp[2], uUpp[2]-uRightp[2],  1.0e-6 );	
	uRightp[3] -= 0.5*Minmod_LimiterA( uRightp[3]-uLeftp[3], uUpp[3]-uRightp[3],  1.0e-6 );
	uRightp[4] -= 0.5*Minmod_LimiterA( uRightp[4]-uLeftp[4], uUpp[4]-uRightp[4],  1.0e-6 );												
												
			
	#return RoeFlux2d(UpRight,UpLeft, nx,ny,side,thermo.Gamma);		
	AUSMplusFlux2dFast(uRightp[1], uRightp[2], uRightp[3], uRightp[4],
			uLeftp[1], uLeftp[2], uLeftp[3], uLeftp[4], nx, ny, side, thermo.Gamma, flux);		
